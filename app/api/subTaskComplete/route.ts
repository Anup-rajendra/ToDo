...
interface SubTaskCount {
  ...
  try {
    ...
    if (data) {
      ...
      const countSubTask: SubTaskCount[] =
        await db.$queryRaw`Select Count(*)::integer AS count from "Task" Where parent_task_id=${mainTaskId}`;
      const countCompletedSubTask: SubTaskCount[] =
        await db.$queryRaw`Select Count(*)::integer AS count from "Task" Where parent_task_id=${mainTaskId} and is_completed=true`;
